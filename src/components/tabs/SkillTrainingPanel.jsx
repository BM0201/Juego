...
function SkillTrainingPanel({ tabLabel, training, onTrainSkill }) {
  const tabSkills = useMemo(
    () => Object.entries(SKILL_DEFINITIONS).filter(([, config]) => config.tabHints.includes(tabLabel)),
    [tabLabel]
  );

  const evaluatedSkills = useMemo(
    () => tabSkills.map(([skillId, config]) => ({ skillId, config, gate: canTrainSkillById({ training, skillId }) })),
    [tabSkills, training]
  );

  if (!training) return null;

  const availableSkills = evaluatedSkills.filter((item) => item.gate.allowed);
  const blockedSkills = evaluatedSkills.filter((item) => !item.gate.allowed);
  ...
  return (
    <section className="card compact">
      ...
      {blockedSkills.length ? (
        <ul className="tiny muted compact-list">
          {blockedSkills.map((item) => <li key={item.skillId}>{item.config.label}: {item.gate.reason}</li>)}
        </ul>
      ) : null}

      {training.lastTrainingFeedback ? (
        <p className="tiny" aria-live="polite"><strong>Última mejora:</strong> {training.lastTrainingFeedback}</p>
      ) : null}
